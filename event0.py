    ...
    def update(self):
        ...
        for _ in range(n):
            ...
            for cogobj_id in self._ref_ls:
                blocking = self._ref_ls[cogobj_id].proc_event(curr_ev)
                if blocking:
                    continue


# ------ avant ct runners.py
class GameTicker(Listener):
...
#  basé sur (GameTicker.
# - - -  - -
class StackBasedGameCtrl(Listener):
